...
- 진단: 앞면 컨테이너는 `px-10`(좌우 40px 대칭)이었지만, 배지·번호·디테일 블록에만 개별적으로 `mr-6`(24px)이 "접힘 보정"이라는 주석과 함께 붙어 있어 실측 좌 40px/우 64px로 어긋나 있었음. 정작 칩을 감싸는 `ul`은 이 보정에서 아예 빠져 있었고, `flex-1`인데 `min-w-0`이 없어 `flex-wrap`이 걸려 있어도 컨테이너 자체가 자식 min-content 폭까지 넓어지려다 옆면 경계를 넘어갈 수 있는 상태였음
- 해결: 개별 `mr-6`들을 모두 제거하고, 옆면(depth face) 폭 상수(`FOLD_W`=40px)를 컨테이너의 `padding-right`에서 한 번만 계산(데스크톱 `pl-10`+`paddingRight:80px`, 모바일 `pl-7`+`paddingRight:68px`) — 앞면 실사용 영역(옆면을 뺀) 기준 좌우 정확히 대칭. 칩 `ul`에 `min-w-0` 추가로 오버플로우 대신 줄바꿈되도록 수정
- 검증: L01~L05 전 레이어에서 `leftGap`/`rightGap`을 `getBoundingClientRect()`로 실측(40 vs 80, 옆면 40px를 빼면 40 vs 40으로 대칭), L05 데스크톱 1300/1050/1025px(lg 최소치)에서 칩이 옆면 경계를 넘지 않고 필요시 줄바꿈되는지, 모바일 375px도 함께 확인 후 커밋·푸시·Vercel 프로덕션 배포

## 2026-08-05 — Framework 슬랩 SVG → 순수 CSS 재구현 (모바일 헤더 오버플로우 + 테두리 두께 불일치, commit 447d418)

- 배경 1(모바일 헤더가 접힘 시임선을 뚫고 나옴): 기존 `LayerSlab`은 카드 외곽선+접힘 시임을 SVG path 하나로, `preserveAspectRatio="none"`으로 비율 무시하고 늘려 그렸음. 시임선 y좌표(뷰박스 기준 전체 높이의 16.16% 지점)가 카드 실제 높이에 비례해 내려가는데, 헤더는 `pt-10`(고정 40px)이라 카드 높이가 ~247px를 넘는 순간부터(칩 2줄·2줄 타이틀 등으로 쉽게 발생) 시임선이 헤더보다 아래로 내려가 헤더가 상대적으로 접힘 영역(시임선 위)에 위치하게 됨 — `getBoundingClientRect()` 실측으로 정확히 재현·확인
- 1차 시도: 접힘 밴드만 고정 높이 SVG(캡)로 분리하고 몸통은 남은 높이를 채우도록 stretch — 오버플로우는 해결됐으나, 캡/몸통 SVG마다 가로:세로 스케일 비율(anisotropy)이 달라 `vector-effect="non-scaling-stroke"`를 써도 앞면 직선 테두리와 대각선 접힘 모서리의 실제 두께가 미묘하게 달라 보이는 문제를 사용자가 재지적(비균일 스케일에서 non-scaling-stroke가 방향별로 다르게 렌더링되는 SVG 렌더링 한계)
- 2차 시도(회귀 발생): SVG를 걷어내고 고정 길이+고정 각도로 회전한 `border-top` div로 대각선을 그리는 방식으로 교체했으나, (a) 우측 옆면 시임 div가 `bottom: FOLD_H`로 24px 짧게 끝나 우하단 접힘과 끊겨 보이고, (b) 좌상단·우하단 대각선은 서로 반대 방향인데 같은 회전각을 그대로 재사용해 모양이 깨지는 회귀 2건 발생 — 스크린샷으로 재현·원인 특정
- 최종 채택: 접힘 크기를 카드 크기와 무관한 고정 픽셀(가로 40px·세로 24px)로 두고, 외곽 실루엣(대각선 모서리 2곳 포함)은 **이중 clip-path 폴리곤**(테두리색 육각형 위에 2px 안쪽으로 당긴 채움색 육각형을 겹치는 방식)으로 통일 — 육각형 전체가 "하나의 폴리곤 정의"라 조인트가 어긋날 수 없고 border-width가 스케일과 무관하게 항상 정확히 2px. (`border` 속성을 clip-path 도형에 직접 얹는 방식은 대각선으로 잘린 가장자리엔 선이 안 그려져서 기각 — clip-path는 페인트를 잘라낼 뿐 새 테두리를 그려주지 않음)
- 부수 회귀(hover 색상): 이중 폴리곤에서 안쪽(채움) 육각형이 바깥쪽(테두리색) 육각형 위에 겹쳐지는 구조인데, 안쪽을 `rgba(255,255,255,0.5)` 반투명으로 칠해 "흰색이 페이지 배경과 섞이길" 의도했으나 실제로는 바로 아래 테두리색과 섞임 — 평소(회색)엔 티가 안 났지만 hover로 테두리가 액센트 블루로 바뀌면 카드 전체가 진한 블루로 물듦. 채움색을 페이지 배경(`--color-bg`)과 미리 섞은 불투명색(`color-mix(in srgb, white 50%, var(--color-bg))`)으로 교체해 테두리색과 무관하게 채움색이 고정되도록 수정
- 검증: 320/375/390/768/1100px 스크린샷 비교, L01~L05 + PHYSICS VALIDATION 레일 호버 전부 확인(채움색 고정·테두리만 점등), 320px 지점의 컴포넌트 왕복 이전엔 재현 못 했던 실기기 케이스라 실측 위주로 검증
